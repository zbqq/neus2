...
if __name__ == '__main__':
	...
	def grad_check_mlp():
		...
		# model = SDF(True, n_levels=1, log2_hashmap_size=15, base_resolution=8, smoothstep=False).to(device)
		def apply_on_x(x):
			params = model.decoder.params.to(_torch_precision(model.decoder.native_tcnn_module.param_precision())).contiguous()
			return _module_function.apply(
				model.decoder.native_tcnn_module, x, params, 128.0
			)
		...
		def backward_apply_on_x(x):
			dL_dy = torch.ones([*x.shape[:-1], model.decoder.n_output_dims], dtype=dtype, device=device)
			params = model.decoder.params.to(_torch_precision(model.decoder.native_tcnn_module.param_precision())).contiguous()
			native_ctx, y = model.decoder.native_tcnn_module.fwd(x, params)
			dummy_ctx_fwd = SimpleNamespace(
				native_tcnn_module=model.decoder.native_tcnn_module,
				loss_scale=model.decoder.loss_scale,
				native_ctx=native_ctx)
			return _module_function_backward.apply(dummy_ctx_fwd, dL_dy, x, params, y)
		...
		def backward_apply_on_params(params):
			x = (torch.tensor([[0.17, 0.55, 0.79]], dtype=torch.float, device=device)).requires_grad_(True)
			dL_dy = torch.ones([*x.shape[:-1], model.decoder.n_output_dims], dtype=dtype, device=device)
			params = params.to(_torch_precision(model.decoder.native_tcnn_module.param_precision())).contiguous()
			native_ctx, y = model.decoder.native_tcnn_module.fwd(x, params)
			dummy_ctx_fwd = SimpleNamespace(
				native_tcnn_module=model.decoder.native_tcnn_module,
				loss_scale=model.decoder.loss_scale,
		...
		# autograd.gradcheck(
		# 	backward_apply_on_dLdy,
		# 	torch.randn([1,model.decoder.n_output_dims], dtype=dtype, device=device).requires_grad_(True),
		# 	eps=1.0e-3, atol=0.01, rtol=0.001
		# )
