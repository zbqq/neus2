...
__global__ void grid_to_bitfield(const uint32_t n_elements,
	...
	// float thresh = NERF_MIN_OPTICAL_THICKNESS();
	// printf("mean_density_ptr:%f\n",*mean_density_ptr);
	float thresh = -1.0f;
...
__global__ void compute_loss_kernel_train_nerf(
	...
	for (uint32_t j = 0; j < compacted_numsteps; ++j) {
		...
		// printf("depth_supervision:%f\n",depth_supervision);
		// printf("dloss_by_drgb: %f,%f,%f\n",dloss_by_drgb.x(),dloss_by_drgb.y(),dloss_by_drgb.z());
		// printf("dloss_dmlp:%f\n",dloss_by_dmlp);
...
void Testbed::update_density_grid_nerf(float decay, uint32_t n_uniform_density_grid_samples, uint32_t n_nonuniform_density_grid_samples, cudaStream_t stream) {
	...
	for (uint32_t i = 0; i < n_steps; ++i) {
		...
		// printf("density_matrix shape:%d,%d\n",density_matrix.m(),density_matrix.n());			
		// printf("density_matrix layout:%d\n",density_matrix.layout());			
